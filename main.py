...
class Tile:
    ...
    def get_color(self):
        color_index = (int(math.log2(self.value)) - 1) % 9
...
def draw(window, tiles, indicator="continue"):

    if indicator == "lost":
        text = FONT.render("You Lost!", 1, FONT_COLOR)
        window.blit(
            text,
            (
                WIDTH / 2 - RECT_WIDTH / 2,
                HEIGHT / 2 - RECT_HEIGHT / 4,
            ),
        )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    end_ind = move_tiles(window, tiles, clock, "left")
                if event.key == pygame.K_RIGHT:
                    end_ind = move_tiles(window, tiles, clock, "right")
                if event.key == pygame.K_UP:
                    end_ind = move_tiles(window, tiles, clock, "up")
                if event.key == pygame.K_DOWN:
                    end_ind = move_tiles(window, tiles, clock, "down")

        draw(window, tiles, end_ind)
